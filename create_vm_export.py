...
from azure.mgmt.compute.models import DataDisk, CreationData
...
import generate_export_setup  # Your PowerShell setup generator module
...
GALLERY_NAME = 'export2vhd'
GALLERY_IMAGE_NAME = 'export2vhd'
GALLERY_IMAGE_VERSION = '1.0.0'
OS_DISK_SSD_GB = '256'
WINDOWS_IMAGE_PASSWORD = 'export2vhd'
DATA_DISK_SSD_GB = '1024' #this will store the downloaded VHD for the conversion
...
async def main():
    ...
    pc_name = ''.join(random.choices(string.ascii_lowercase, k=6)) + "idtech"
    ...
    print_info(f"Getting VM '{vm_name}'...")
    ...
    sas_expiry_hours = 10  # or your preferred duration
    try:
        print_info("Generating SAS URL via az CLI...")
        cmd = [
            "az", "snapshot", "grant-access",
            "--resource-group", resource_group,
            "--name", snapshot_name,
            "--duration-in-seconds", str(sas_expiry_hours * 3600),
            "--query", "accessSas",
            "-o", "tsv"
        ]
        download_sas_url = subprocess.check_output(cmd, text=True).strip()
        print_success(f"SAS URL generated (valid for {sas_expiry_hours} hours):\n{download_sas_url}")
    except Exception as e:
        print_error(f"Failed to generate SAS URL using az CLI: {e}")
        sys.exit(1)

    container_name = "vhd-uploads"
    blob_name = f"{vm_name}-fixed.vhd"  # target blob name for upload
    ...
    upload_sas_url = f"https://{storage_account_name}.blob.core.windows.net/{container_name}/{blob_name}?{upload_sas_token}"
    print_info(f"SAS URL for uploading fixed VHD:\n{upload_sas_url}")
    ...
    hook_url = "", 
    download_folder= "D:\\"
    ps_script = generate_export_setup.generate_hyperv_setup_script(
        os_disk_sas_url=download_sas_url,
        fixed_vhd_upload_sas_url=upload_sas_url,
        hook_url=hook_url,
    ...
    # Create managed disk for data disk
    data_disk_name = f"{vm_name}-data-disk"
    data_disk_params = {
        'location': location,
        ...
        'disk_size_gb': int(DATA_DISK_SSD_GB)  
    }
    data_disk = compute_client.disks.begin_create_or_update(resource_group, data_disk_name, data_disk_params).result()

    # Prepare OS disk (same as your existing code)
    os_disk = {
        ...
        'disk_size_gb': int(OS_DISK_SSD_GB)
    }
    ...
    # Trusted Launch profile (same)
    security_profile = SecurityProfile(
        security_type="TrustedLaunch"
    )

    # Attach data disk (lun 0 is usually first data disk)
    data_disk_attachment = DataDisk(
        lun=0,
        name=data_disk.name,
    ...
    # Now create VM parameters including the data disk
    vm_parameters = VirtualMachine(
        ...
        storage_profile=StorageProfile(
            ...
            data_disks=[data_disk_attachment]
        ),
    ...
    # Create DNS A record [NO A_RECORD_SET_NEEDED]
    # a_records = [f'@']
