...
GALLERY_IMAGE_NAME = 'lithtech'
GALLERY_IMAGE_VERSION = '1.0.2'
OS_DISK_SSD_GB = '256'
WINDOWS_IMAGE_PASSWORD = 'xboxDevKitRTX1!'
...
async def main():
    ...
    pc_name = ''.join(random.choices(string.ascii_lowercase, k=6)) + "lithtech"
    ...
    if subdomain:
        ...
        fqdn_name = domain
    print_info(f"Full domain to configure: {fqdn_name}")
    ...
    ps_script = generate_setup.generate_setup(vm_name, fqdn_name,ssl_email, DUMBDROP_PIN, WINDOWS_IMAGE_PASSWORD)
   ...
   # Define Trusted Launch security profile
    ...
    # Create DNS A record for 'pin.domain' 'drop.domain'
    a_records = [f'pin.{subdomain}',f'drop.{subdomain}',f'web.{subdomain}']
    ...
    if extension:
        ...
        print_success("-----------------------------------------------------")
        # Construct the URL
        url = f"https://cdn.sdappnet.cloud/rtx/rtxue3.html?url={public_ip}"
