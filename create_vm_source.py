...
GALLERY_IMAGE_NAME = 'rtx2source'
GALLERY_IMAGE_VERSION = '1.0.2'
OS_DISK_SSD_GB = '256'
WINDOWS_IMAGE_PASSWORD = 'sourceDevKitRTX1!'
...
async def main():
    ...
    pc_name = ''.join(random.choices(string.ascii_lowercase, k=6)) + "source"
   ...
   # Define Trusted Launch security profile
    ...
    if extension:
        ...
        print_success(f"https://pin.{fqdn}")
        print_success(f"Drop files service at:-----------------------------")
        print_success(f"https://drop.{fqdn}")
        print_success(f"Pin: {DUMBDROP_PIN}")
        print_success("-----------------------------------------------------")
        print_success(f"https://cdn.sdappnet.cloud/rtx/rtxsource.html?url={public_ip}")
        print_success("-----------------------------------------------------")
        # Construct the URL
        url = f"https://cdn.sdappnet.cloud/rtx/rtxsource.html?url={public_ip}"
        ...
        html_content = html_email.HTMLEmail(
            ...
            title=f"{vm_name} - Source Engine",
            main_heading=f"{vm_name} - Source Engine",
